	...
	$enc_key  = $settings['enc_key'];
        ...
        if(isset($_SESSION['gpx_admin']))
        {
            ...
            if($cur_username != $username || !empty($password))
            {
                ...
                $GLOBALS['mysqli']->query("UPDATE users SET `setpass_3010` = '0', last_updated = NOW(),theme = '$theme',sso_user = AES_ENCRYPT('$sso_user', '$enc_key'),language = '$language',username = '$username',email_address = '$email',first_name = '$first_name',last_name = '$last_name'$sql_pass WHERE id = '$userid'") or die('Failed to update user');
            }
            // Otherwise update basic settings
            else
            {
                $GLOBALS['mysqli']->query("UPDATE users SET `setpass_3010` = '0', last_updated = NOW(),theme = '$theme',language = '$language',email_address = '$email',first_name = '$first_name',last_name = '$last_name' WHERE id = '$userid'") or die('Failed to update user');
            }
        }
        ...
        {
            $GLOBALS['mysqli']->query("UPDATE users SET `setpass_3010` = '0',last_updated = NOW(),theme = '$theme',language = '$language',email_address = '$email',first_name = '$first_name',last_name = '$last_name'$sql_pass WHERE id = '$userid'") or die('Failed to update your account!');
